# 불법 복제 사이트 다운로더
![downloads](https://img.shields.io/github/downloads/deltoid7/tvroom-downloader/total?label=downloads&color=blue&logo=github)
![badge](https://img.shields.io/badge/status-active-brightgreen)<br>
불법 복제된 콘텐츠를 불법 복제하기로 했습니다.<br>
트래픽을 증가시키고, 광고 효율을 줄임으로써 많은 불법 복제 사이트들이 사라졌으면 좋겠습니다. (사실 그럴 가능성은 낮지만요.)

`현재 다운로더 완성 목록: 티비룸, 티비위키, 짭플릭스, 코티비씨`

<br>

## 설명서
[릴리스 페이지](https://github.com/deltoid7/piracy-downloader/releases)로 이동하여 프로그램을 다운로드 받습니다.<br>
또는 아래 리스트에서 원하는 프로그램 링크 클릭해서 다운로드 받습니다.

티비룸: https://github.com/deltoid7/piracy-downloader/releases/download/tvroom/tvroom_v1.exe<br>
`다른 사이트 준비 중..`

[ffmpeg](https://ffmpeg.org/)를 다운로드 받습니다.<br>
실행하고, 해당 사이트에서 보고 있던 url을 붙여 넣으세요.<br>
url이 망가져 보여도 괜찮습니다.<br>
(예시: `https://tvroom.cc/video/%EA%B8%B0%EC%83%9D%EC%B6%A9-2019/%EB%B3%B8%ED%8E%B8`)
<br>
<br>
단기간에 요청을 너무 많이 날리면 차단당합니다.<br>
ip 기반 차단이니 vpn을 쓰시거나 [cloudflare warp](https://one.one.one.one/) 사용을 권장합니다.

<br>

## 공지
도움이 됐고, 연락처로 컨택 후 기부해 주실 아량이 있다면 베풀어주시면 감사하겠습니다.<br>
전에 티비위키 버전도 494분이 다운로드 하셨는데, 구매하신 분은 몇 없으셨어요.. (참고로 현재는 WASM 기반 자체 DRM 사용 중)
<img width="1701" height="346" alt="image" src="https://github.com/user-attachments/assets/e9e7840b-0974-4fb8-bfd4-1f043110230b" />

<br>

## 연락처
여러 잡다한 문의 가능<br>
완성하고 빨리빨리 업로드 안하는 편이라 필요한 것 있으면 연락하세요.

![](https://dcbadge.limes.pink/api/shield/1285574573877624924?style=flat?d=z)
[![](https://img.shields.io/badge/DongjuTheDevloper-26A5E4?style=flat&logo=telegram&logoColor=white)](https://t.me/DongjuTheDeveloper)
![](https://img.shields.io/badge/judong1094@gmail.com-ffffff.svg?&logo=gmail)
![](https://img.shields.io/badge/dongjuthedeveloper@gmail.com-ffffff.svg?&logo=gmail)
